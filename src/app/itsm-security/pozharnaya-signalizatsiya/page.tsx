...
export default function PozharnayaSignalizatsiyaPage() {
    ...
    const services = [
        ...
        'СЕРВИСНОЕ ТЕХНИЧЕСКОЕ ОБСЛУЖИВАНИЕ ПОЖАРНОЙ СИГНАЛИЗАЦИИ'
    ];

    const benefits = [
        'СЕРТИФИЦИРОВАННЫЕ СПЕЦИАЛИСТЫ',
        'ЛИЦЕНЗИЯ МЧС',
        'ИМЕЕМ ВСЕ НЕОБХОДИМЫЕ ДОПУСКИ И РАЗРЕШЕНИЯ',
        ...
        <div className={styles.page}>
            ...
            <div className={`${styles.hero} ${isLoading ? styles.loading : ''}`}>
                ...
                <p className={`${styles.subtitle} ${isLoading ? styles.loading : ''}`}>
                    ПОЖАРНАЯ СИГНАЛИЗАЦИЯ В КРАСНОЯРСКЕ И БЛИЖАЙШИХ ГОРОДАХ ОТ КОМПАНИИ «АйТиСМ»
                </p>
            </div>

            {/* Секция услуг */}
            <div className={`${styles.servicesSection} ${isLoading ? styles.loading : ''}`}>
                <h2 className={`${styles.servicesTitle} ${isLoading ? styles.loading : ''}`}>
                    ВСЕ ВИДЫ РАБОТ:
                </h2>
